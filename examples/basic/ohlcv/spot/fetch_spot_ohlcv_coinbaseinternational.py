#!/usr/bin/env python3
"""
Basic example of fetching OHLCV (Open, High, Low, Close, Volume) data from Coinbase International.
This script demonstrates how to:
1. Initialize the Coinbase International connection
2. Fetch OHLCV data for a specific symbol
3. Convert the data to a pandas DataFrame for analysis
4. Handle basic error cases

Note: This uses Coinbase International (formerly Coinbase Pro International) which is different from 
the regular Coinbase API and Coinbase Advanced Trade API.
"""
...
def fetch_ohlcv(symbol='BTC/USD', timeframe='1h', limit=100, max_retries=3):
    """
    Fetch OHLCV data from Coinbase International for a given symbol and timeframe.
    ...
    for attempt in range(max_retries):
        try:
            # Initialize Coinbase International with configuration
            exchange = ccxt.coinbaseinternational({
                ...
                'options': {
                    ...
                    'tradesLimit': 1000,  # Increase trades limit for better OHLCV data
                },
                'headers': {
            ...
            if symbol not in exchange.markets:
                ...
                available_symbols = [s for s in exchange.markets.keys() if any(stable in s for stable in ['/USD', '/USDT', '/USDC'])][:5]
...
def main():
    # Example usage
    symbol = 'BTC/USD'  # Coinbase International uses USD pairs
    timeframe = '1h'
    limit = 100
    
    print(f"[yellow]Fetching {timeframe} OHLCV data for {symbol} from Coinbase International...[/yellow]")
