#!/usr/bin/env python3
"""
Basic example of fetching OHLCV (Open, High, Low, Close, Volume) data from KuCoin Spot.
This script demonstrates how to:
1. Initialize the KuCoin exchange
2. Fetch OHLCV data for a specific symbol
3. Convert the data to a pandas DataFrame for analysis
4. Handle basic error cases

Note: KuCoin primarily uses USDT pairs for spot trading, with some USDC and BTC pairs available.
The exchange may have rate limits and requires specific error handling for timeframes.
"""
...
def fetch_ohlcv(symbol='BTC/USDT', timeframe='1h', limit=100, max_retries=3):
    ...
    for attempt in range(max_retries):
        try:
            # Initialize KuCoin exchange with configuration
            exchange = ccxt.kucoin({
                ...
                'options': {
                    ...
                    'recvWindow': 5000,  # KuCoin specific parameter
                },
                'headers': {
            ...
            # Try alternatives if symbol not found
                ...
                base = symbol.split('/')[0]
                # KuCoin primarily uses USDT, then USDC, then BTC
                alternative_symbols = [f"{base}/USDT", f"{base}/USDC", f"{base}/BTC"]
...
def main():
    # Example usage
    symbol = 'BTC/USDT'  # KuCoin primarily uses USDT pairs
    timeframe = '1h'
    limit = 100
    
    print(f"[yellow]Fetching {timeframe} OHLCV data for {symbol} from KuCoin Spot...[/yellow]")
