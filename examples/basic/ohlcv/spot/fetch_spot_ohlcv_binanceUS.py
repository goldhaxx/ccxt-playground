#!/usr/bin/env python3
"""
Basic example of fetching OHLCV (Open, High, Low, Close, Volume) data from Binance.US.
This script demonstrates how to:
1. Initialize the Binance.US exchange
...
def fetch_ohlcv(symbol='BTC/USDT', timeframe='1h', limit=100, max_retries=3):
    """
    Fetch OHLCV data from Binance.US for a given symbol and timeframe.
    ...
    for attempt in range(max_retries):
        try:
            # Initialize Binance.US exchange with configuration
            exchange = ccxt.binanceus({
                ...
                'options': {
                    ...
                    'recvWindow': 5000,  # increase receive window
                },
                'headers': {
            ...
            # Direct klines endpoint for debugging
            klines_url = f"{exchange.urls['api']}/api/v3/klines?symbol={exchange_symbol}&interval={timeframe}&limit={limit}"
            print(f"\n[bold]Equivalent REST API endpoint:[/bold]\n{klines_url}")
...
def main():
    ...
    print(f"[yellow]Fetching {timeframe} OHLCV data for {symbol} from Binance.US...[/yellow]")
