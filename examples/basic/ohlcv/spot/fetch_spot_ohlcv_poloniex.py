#!/usr/bin/env python3
"""
Basic example of fetching OHLCV (Open, High, Low, Close, Volume) data from Poloniex.
This script demonstrates how to:
1. Initialize the Poloniex exchange
2. Fetch OHLCV data for a specific symbol
3. Convert the data to a pandas DataFrame for analysis
4. Handle basic error cases

Note:
    Poloniex primarily uses USDT as the quote currency.
    Some legacy pairs might use BTC as the quote currency.
"""
...
def fetch_ohlcv(symbol='BTC/USDT', timeframe='1h', limit=100, max_retries=3):
    """
    Fetch OHLCV data from Poloniex for a given symbol and timeframe.
    
    Args:
        symbol (str): Trading pair symbol (e.g., 'BTC/USDT')
                     Note: Poloniex primarily uses USDT pairs
    ...
    for attempt in range(max_retries):
        try:
            # Initialize Poloniex exchange with configuration
            ...
            # Try alternatives if USDT pair not found
            elif symbol.endswith('/USDT'):
                base = symbol.split('/')[0]
                alternative_symbols = [f"{base}/BTC", f"{base}/USD"]
                print(f"[yellow]USDT pair not found. Trying alternatives: {', '.join(alternative_symbols)}[/yellow]")
            ...
            if symbol not in exchange.markets:
                ...
                if usdt_pairs:
                    print(f"Available USDT pairs: {', '.join(usdt_pairs)}...")
                if btc_pairs:
...
def main():
    # Example usage
    symbol = 'BTC/USDT'  # Poloniex primarily uses USDT pairs
    timeframe = '1h'
    limit = 100
    
    print(f"[yellow]Fetching {timeframe} OHLCV data for {symbol} from Poloniex...[/yellow]")
    print("[yellow]Note: Poloniex primarily uses USDT pairs, with some legacy BTC pairs[/yellow]")
