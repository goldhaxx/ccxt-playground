#!/usr/bin/env python3
"""
Basic example of fetching OHLCV (Open, High, Low, Close, Volume) data from Coinbase Exchange (Advanced Trade API).
This script demonstrates how to:
1. Initialize the Coinbase Exchange connection
2. Fetch OHLCV data for a specific symbol
3. Convert the data to a pandas DataFrame for analysis
4. Handle basic error cases

Note: This uses the Advanced Trade API (coinbaseexchange) which is different from the regular Coinbase API.
"""
...
def fetch_ohlcv(symbol='BTC/USD', timeframe='1h', limit=100, max_retries=3):
    """
    Fetch OHLCV data from Coinbase Exchange for a given symbol and timeframe.
    ...
    for attempt in range(max_retries):
        try:
            # Initialize Coinbase Exchange with configuration
...
def main():
    # Example usage
    symbol = 'BTC/USD'  # Coinbase Exchange uses USD pairs
    timeframe = '1h'
    limit = 100
    
    print(f"[yellow]Fetching {timeframe} OHLCV data for {symbol} from Coinbase Exchange (Advanced Trade)...[/yellow]")
