#!/usr/bin/env python3
"""
Basic example of fetching OHLCV (Open, High, Low, Close, Volume) data from BitMart.
This script demonstrates how to:
1. Initialize the BitMart exchange
...
def fetch_ohlcv(symbol='BTC/USDT', timeframe='1h', limit=100, max_retries=3):
    """
    Fetch OHLCV data from BitMart for a given symbol and timeframe.
    ...
    for attempt in range(max_retries):
        try:
            # Initialize BitMart exchange with configuration
            exchange = ccxt.bitmart({
                ...
                'options': {
                    ...
                    'defaultTimeInForce': 'GTC',  # Good Till Cancel order type
                },
                'headers': {
...
def main():
    ...
    print(f"[yellow]Fetching {timeframe} OHLCV data for {symbol} from BitMart...[/yellow]")
