#!/usr/bin/env python3
"""
Basic example of fetching OHLCV (Open, High, Low, Close, Volume) data from Blofin.
This script demonstrates how to:
1. Initialize the Blofin exchange
...
def fetch_ohlcv(symbol='BTC/USDT', timeframe='1h', limit=100, max_retries=3):
    """
    Fetch OHLCV data from Blofin for a given symbol and timeframe.
    ...
    for attempt in range(max_retries):
        try:
            # Initialize Blofin exchange with configuration
            exchange = ccxt.blofin({
                ...
                'options': {
                    ...
                    'createMarketBuyOrderRequiresPrice': True,  # Blofin specific requirement
                },
                'headers': {
            ...
            # Handle Blofin's specific symbol format for USDT pairs
            original_symbol = symbol
            if symbol.endswith('/USDT') and not symbol.endswith(':USDT'):
                symbol = f"{symbol}:USDT"
                print(f"[yellow]Converting {original_symbol} to Blofin format: {symbol}[/yellow]")
...
def main():
    ...
    print(f"[yellow]Fetching {timeframe} OHLCV data for {symbol} from Blofin...[/yellow]")
