#!/usr/bin/env python3
"""
Basic example of fetching OHLCV (Open, High, Low, Close, Volume) data from MEXC Spot.
This script demonstrates how to:
1. Initialize the MEXC exchange
2. Fetch OHLCV data for a specific symbol
3. Convert the data to a pandas DataFrame for analysis
4. Handle basic error cases

Note: MEXC primarily uses USDT pairs for spot trading, with some USDC pairs available.
The exchange has specific rate limits and may require API keys for some endpoints.
"""
...
def fetch_ohlcv(symbol='BTC/USDT', timeframe='1h', limit=100, max_retries=3):
    """
    Fetch OHLCV data from MEXC for a given symbol and timeframe.
    ...
    for attempt in range(max_retries):
        try:
            # Initialize MEXC exchange with configuration
            exchange = ccxt.mexc({
                ...
                'options': {
                    ...
                    'createMarketBuyOrderRequiresPrice': True,  # MEXC specific requirement
            ...
            # Try alternatives if symbol not found
                ...
                base = symbol.split('/')[0]
                # MEXC primarily uses USDT, then USDC
            ...
            print("1. Rate limiting (MEXC has specific rate limits)")
...
def main():
    # Example usage
    symbol = 'BTC/USDT'  # MEXC primarily uses USDT pairs
    timeframe = '1h'
    limit = 100
    
    print(f"[yellow]Fetching {timeframe} OHLCV data for {symbol} from MEXC Spot...[/yellow]")
