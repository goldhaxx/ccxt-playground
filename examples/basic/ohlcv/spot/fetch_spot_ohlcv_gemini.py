#!/usr/bin/env python3
"""
Basic example of fetching OHLCV (Open, High, Low, Close, Volume) data from Gemini.
This script demonstrates how to:
1. Initialize the Gemini exchange
2. Fetch OHLCV data for a specific symbol
3. Convert the data to a pandas DataFrame for analysis
4. Handle basic error cases

Note: Gemini uses slightly different symbol formats (e.g., 'btcusd' instead of 'BTC/USD')
and may require authentication for certain endpoints.
"""
...
def fetch_ohlcv(symbol='BTC/USD', timeframe='1h', limit=100, max_retries=3):
    """
    Fetch OHLCV data from Gemini for a given symbol and timeframe.
    ...
    for attempt in range(max_retries):
        try:
            # Initialize Gemini exchange with configuration
            ...
            # Only try alternatives if USD pair not found
                ...
                base = symbol.split('/')[0]
                alternative_symbols = [f"{base}/USDT", f"{base}/DAI"]  # Gemini uses USD, USDT, and DAI pairs
            ...
            if symbol not in exchange.markets:
                ...
                available_symbols = [s for s in exchange.markets.keys() if any(stable in s for stable in ['/USD', '/USDT', '/DAI'])][:5]
            ...
            print("5. Symbol format (Gemini uses lowercase symbols without slashes)[/yellow]")
            return None
...
def main():
    # Example usage
    symbol = 'BTC/USD'  # Gemini uses USD as primary quote currency
    timeframe = '1h'
    limit = 100
    
    print(f"[yellow]Fetching {timeframe} OHLCV data for {symbol} from Gemini...[/yellow]")
