#!/usr/bin/env python3
"""
Basic example of fetching OHLCV (Open, High, Low, Close, Volume) data from BingX.
This script demonstrates how to:
1. Initialize the BingX exchange
...
def fetch_ohlcv(symbol='BTC/USDT', timeframe='1h', limit=100, max_retries=3):
    """
    Fetch OHLCV data from BingX for a given symbol and timeframe.
    ...
    for attempt in range(max_retries):
        try:
            # Initialize BingX exchange with configuration
            exchange = ccxt.bingx({
...
def main():
    ...
    print(f"[yellow]Fetching {timeframe} OHLCV data for {symbol} from BingX...[/yellow]")
